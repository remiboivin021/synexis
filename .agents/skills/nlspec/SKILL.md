---
name: nlspec
description: Produce an authoritative, implementable, testable NLSpec.
---
...
The NLSpec is authoritative for behavior and constraints inside its approved scope.
If implementation diverges from NLSpec, that divergence must be explicitly resolved by the workflow authorities.
...
- NLSpec is a precision artifact, not a brainstorming document.
...
- No architecture redesign without escalation.
- No speculative behavior outside provided scope.
- No invented repository facts.
- Prefer precision over verbosity and general theory.

MISSIONS (MANDATORY)
1) Convert request intent into a deterministic, testable product contract.
2) Define explicit scope boundaries and explicit non-goals with no ambiguity.
...
5) Define failure modes and required system behavior for each relevant class of failure.
6) State assumptions and open questions explicitly when evidence is missing.
7) Align specification with existing architecture, governance, and repository constraints.
8) Prevent speculative requirements that are not grounded in approved scope.
9) Produce a complete Definition of Done tied to concrete validation checks.
10) Ensure the spec can be consumed directly by planner/coder/qa without reinterpretation.
11) Flag contract or architectural implications that require ADR or authority escalation.
12) Keep wording normative, stable, and suitable as an implementation reference.
...
- The feature / component request (user prompt)
- Repository structure and relevant docs (if available)
- Existing contracts (config/API/schema) if any

If filesystem access is unavailable:
Ask the user to paste only the minimum necessary context (1–3 requests max).
...
- Do not invent repository evidence. If unknown, mark as Assumption or Open Question.
...
The sections may be expanded if necessary, but must not be removed.

If .agents/templates/NLSPEC.template.md exists, it MUST be used.
...
# Spec quality checklist (internal)

Before finalizing:
- Does every major behavior have a deterministic rule?
- Are defaults specified?
- Are constraints testable?
- Is DoD directly actionable (commands/tests/lints)?
- Are trust boundaries explicit when external IO exists?
- Are backward compatibility rules stated for public contracts?
